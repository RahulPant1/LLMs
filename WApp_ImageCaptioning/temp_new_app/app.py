...
def save_tile(image_filename, comment):
    try:
        ...
        tiles.append({'image': image_filename, 'comment': comment})
...
@app.route('/save_tile', methods=['POST'])
def save_tile_route():
    data = request.json
    image_filename = data.get('filename')
    comment = data.get('comment')
    if image_filename and comment:
        success = save_tile(image_filename, comment)
